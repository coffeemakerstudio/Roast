...
export const engine = {
	...
	createEntity<T>(settings: T): T { assertJsonValue(settings); return structuredClone(settings); },
	/** Creates a detached JSON-safe generic structure/geometry authoring record. */
	createStructure<T>(settings: T): T { assertJsonValue(settings); return structuredClone(settings); },
	/** Creates a detached JSON-safe generic effect authoring record. */
	createEffect<T>(settings: T): T { assertJsonValue(settings); return structuredClone(settings); },
...
export type { BinarySchema as GenericBinarySchema, BinarySchemaCapabilities, BinaryView, RegisteredBinarySchema } from "./schema.js";
...
export type { BinarySchemaIdentity, PackedComponentBinding, PackedTypedSection, PackedSnapshotOptions, PackedSchemaDiagnostic, PackedDiagnostics, PackedSnapshotDiagnosticsResult } from "./binary.js";
export type { BinarySchema } from "./schema.js";
export type { BinaryStorage, BinarySnapshotFormat } from "./binary.js";
