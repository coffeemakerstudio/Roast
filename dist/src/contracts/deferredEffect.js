...
export const DEFERRED_EFFECT_SCHEMA_VERSION = 1;
export const DEFERRED_EFFECT_DURATION_UNITS = ["ticks"];
export function createDeferredEffectTemplate(input) {
    const template = structuredClone(input);
    validateDeferredEffectTemplate(template);
    return template;
}
export function createDeferredEffect(input) {
    const deferred = {
...
export function advanceDeferredEffect(effect) {
...
export function validateDeferredEffectTemplate(value) {
...
export function validateDeferredEffect(value) {
    if (!value || typeof value !== "object" || Array.isArray(value))
